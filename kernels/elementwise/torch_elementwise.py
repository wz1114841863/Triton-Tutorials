...
torch::Tensor launch_add(torch::Tensor x, torch::Tensor y) {
    ...
    element_ops_kernel<<<grid_size, block_size>>>(
        x.data_ptr<float>(), y.data_ptr<float>(), z.data_ptr<float>(), n, AddFunctor()
    );
    return z;
}
...
torch::Tensor launch_mul(torch::Tensor x, torch::Tensor y) {
    ...
    // 实例化:装上 MulFunctor
    element_ops_kernel<<<grid_size, block_size>>>(
        x.data_ptr<float>(), y.data_ptr<float>(), z.data_ptr<float>(), n, MulFunctor()
    );
    return z;
}
"""
...
# 测试加法
z_add = module.launch_add(x, y)
assert torch.allclose(z_add, x + y)
print("✅ Add Match!")

# 测试乘法
z_mul = module.launch_mul(x, y)
assert torch.allclose(z_mul, x * y)
print("✅ Mul Match!")
