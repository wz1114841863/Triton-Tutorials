...
/*
这个 Kernel 的设计思路是非常经典的 "分治法" (Divide and Conquer):
    Thread 级: 每个线程加载一个数据.
    Warp 级: 32 个线程合并成 1 个值(存在 Warp Leader 手里).
    Block 级: 多个 Warp Leader 的值再合并成 1 个值(存在 Block Leader 手里).
    Grid 级: 多个 Block Leader 的值合并到全局内存(通过原子操作).
*/
...
__global__ void block_all_reduce_sum_f32_f32_kernel(float *a, float *y, int N) {
    int tid = threadIdx.x;  // block中的线程id
    int idx = blockIdx.x * NUM_THREADS + tid;  // 全局数据索引
    // 计算block中有多少个warp
    // (256 + 32 - 1) / 32 = 8 个 Warp.constexpr 表示编译时就算好了,不占运行时开销.
    constexpr int NUM_WARPS = (NUM_THREADS + WARP_SIZE - 1) / WARP_SIZE;
    // 申请共享内存.注意:只申请了NUM_WARPS (8) 个float
    __shared__ float reduce_smem[NUM_WARPS];
    // keep the data in register is enough for warp operaion.
    // 这是一个 "One-Thread-One-Element" 的模型,没有 while 循环.
    float sum = (idx < N) ? a[idx] : 0.0f;
    // 计算Warp ID (第几组) 和 Lane ID (组内第几号)
    int warp = tid / WARP_SIZE;
    int lane = tid % WARP_SIZE;
    // perform warp sync reduce.
    // 调用辅助函数,利用寄存器洗牌 (Shuffle),把本 Warp 内 32 个线程的数据加在一起.
    // 执行完这句后,sum 在逻辑上已经包含了本 Warp 的总和
    sum = warp_reduce_sum_f32<WARP_SIZE>(sum);
    // warp leaders store the data to shared memory.
    if (lane == 0)
        // 只有每个 Warp 的lane 0需要把结果写到Shared Memory上.
        // 其他组员的数据已经被加进去了,不用管.
        reduce_smem[warp] = sum;
    __syncthreads(); // make sure the data is in shared memory.
    // the first warp compute the final sum.
    // 现在 Shared Memory 里有 8 个值 (假设 NUM_THREADS=256).
    // 我们只需要一个 Warp (32个线程) 就能处理这 8 个数.
    // 让 Warp 0 的前 8 个线程 (lane < 8) 去读这 8 个值.
    sum = (lane < NUM_WARPS) ? reduce_smem[lane] : 0.0f;
    // 只有 Warp 0 需要干活
    if (warp == 0)
        // 再次调用 Warp Reduce.
        // 这次是在 Warp 0 内部,把刚才读到的 8 个 Warp 总和再加一次.
        // 此时,sum 变成了整个 Block 的总和.
...
template <const int NUM_THREADS = 256 / 8>  // 线程数只需要原来的1/8
