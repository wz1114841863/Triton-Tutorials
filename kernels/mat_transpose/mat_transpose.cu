...
// work for row == col
/*
针对的是 DRAM Partition Camping (分区竞争) 问题
在早期的 GPU 上,如果你按顺序处理 Block (0,0), (0,1), (0,2)...,可能会导致所有 Block 同时在访问显存的同一个 Partition(物理内存条的同一个区域),导致排队.
解决方法是让 Block 斜对角线式地访问数据,这样每个 Block 访问的数据会分布在不同的 Partition 上,减少竞争.
在现代 GPU(Pascal 之后)上,内存控制器的哈希映射已经做得很好,这个优化的收益不再明显.
*/
...
/*
    难点: 如何在Shared Memory里把"横着的 float4" 变成 "竖着的 float4"?
*/
__global__ void mat_transpose_f32x4_shared_col2row2d_kernel(float *x, float *y,
  ...
  if (global_x * 4 + 3 < col + 3 && global_y < row) {
    ...
    // // global_x 是 float4 的索引,所以对应 float 的列索引是 global_x * 4
    float4 x_val = reinterpret_cast<float4 *>(x)[global_y * col / 4 + global_x];
    // local_y: 0~15 (行)
    // local_x: 0~15 (float4 列) -> 对应 float 列 0, 4, 8...
    // 数据在 Shared Memory 里是行优先 (Row-Major) 连续存放的.
    ...
    constexpr int STRIDE = WARP_SIZE_S / 4;  // STRIDE = 16 / 4 = 4
    // 这里的索引计算是为了 "竖着抓 4 个数"
...
__global__ void mat_transpose_f32x4_shared_bcf_col2row2d_kernel(float *x,
  ...
  const int local_x = threadIdx.x;
  const int local_y = threadIdx.y;
  // 原本的列数是 16 * 4 = 64. 64 是 32 (Bank数量) 的倍数.
  // 如果不加 Padding,列读取时一定会发生冲突.
  // 加了 PAD (1) 之后,列数变成 65,完美错开 Bank.
